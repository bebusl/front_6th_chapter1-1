...
${e?`  <div class="space-y-2">
  ...
  `);return r}function S(){let e=()=>a(`
    ...
    </div>`);i({component:e}).render()}const C=()=>r(async()=>{let{worker:e}=await import(`./browser-CsBWiKON.js`);return{worker:e}},[]).then(({worker:e})=>e.start({onUnhandledRequest:`bypass`}));function w(){let e=[{pathname:`/`,component:b,matchPattern:`^/$`},{pathname:`/product/:productId`,component:o,matchPattern:`^/product/[A-Za-z0-9_-]+$`},{pathname:`*`,component:S,matchPattern:`^.*$`}],t=()=>{let t=location.pathname,n=e.find(e=>{let n=new RegExp(e.matchPattern);return n.test(t)});return n?n.component:null};m.onChange(()=>{let e=t()();console.log(e)});let n=t()();console.log(n)}C().then(w);